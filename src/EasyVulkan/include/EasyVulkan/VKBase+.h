...
namespace vulkan {
	...
	public:
		...
		//Getter
		operator VkQueryPool() const { return mqueryPool; }
		const VkQueryPool* Address() const { return mqueryPool.Address(); }
		...
		void CmdCopyResults(VkCommandBuffer commandBuffer, uint32_t firstQueryIndex, uint32_t queryCount, VkBuffer buffer_dst, VkDeviceSize offset_dst, VkDeviceSize stride) const {
			mqueryPool.CmdCopyResults(commandBuffer, firstQueryIndex, queryCount, buffer_dst, offset_dst, stride, VK_QUERY_RESULT_WAIT_BIT);
		}
		//Non-const Function
		void Create(uint32_t capacity) {
			passingSampleCounts.resize(capacity);
			passingSampleCounts.shrink_to_fit();
			mqueryPool.Create(VK_QUERY_TYPE_OCCLUSION, Capacity());
		}
		...
		result_t GetResults(uint32_t queryCount) {
			return mqueryPool.GetResults(0, queryCount, queryCount * 4, passingSampleCounts.data(), 4);
		}
	};
	...
	public:
		...
		//Const Function
		void CmdReset(VkCommandBuffer commandBuffer) const {
			mqueryPool.CmdReset(commandBuffer, 0, 1);
		}
		void CmdBegin(VkCommandBuffer commandBuffer) const {
			mqueryPool.CmdBegin(commandBuffer, 0);
		}
		void CmdEnd(VkCommandBuffer commandBuffer) const {
			mqueryPool.CmdEnd(commandBuffer, 0);
		}
		...
		//Non-const Function
		void Create() {
			mqueryPool.Create(VK_QUERY_TYPE_PIPELINE_STATISTICS, 1, (1 << statisticCount) - 1);
		}
		result_t GetResults() {
			return mqueryPool.GetResults(0, 1, sizeof statistics, statistics, sizeof statistics);
		}
	};
	...
	public:
		...
		void CmdWriteTimestamp(VkCommandBuffer commandBuffer, VkPipelineStageFlagBits pipelineStage, uint32_t queryIndex) const {
			mqueryPool.CmdWriteTimestamp(commandBuffer, pipelineStage, queryIndex);
		}
		//Non-const Function
		void Create(uint32_t capacity) {
			timestamps.resize(capacity);
			timestamps.shrink_to_fit();
			mqueryPool.Create(VK_QUERY_TYPE_TIMESTAMP, Capacity());
		}
		void Recreate(uint32_t capacity) {
			graphicsBase::Base().WaitIdle();
			mqueryPool.~queryPool();
		...
		result_t GetResults(uint32_t queryCount) {
			return mqueryPool.GetResults(0, queryCount, queryCount * 4, timestamps.data(), 4);
		}
	};
}