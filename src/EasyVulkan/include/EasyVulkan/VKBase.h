...
//		fence() = default;
        ...
        explicit fence(VkDevice* device, VkFenceCreateFlags flags = 0): fence(device, VkFenceCreateInfo{.flags=flags}) {}
    ...
    public:
        graphicsBase() {
            this->AddCallback_CreateDevice([this] { this->Initialize(); });
            this->AddCallback_DestroyDevice([this] { this->Cleanup(); });
        }

        void Initialize() {
            commandPool_graphics = new commandPool(&device);
            commandPool_presentation = new commandPool(&device);
            commandPool_compute = new commandPool(&device);
            commandBuffer_transfer = new commandBuffer();
            commandBuffer_presentation = new commandBuffer();
            if (this->QueueFamilyIndex_Graphics() != VK_QUEUE_FAMILY_IGNORED) {
                commandPool_graphics->Create(this->QueueFamilyIndex_Graphics(),
            ...
            if (this->QueueFamilyIndex_Compute() != VK_QUEUE_FAMILY_IGNORED)
                commandPool_compute->Create(this->QueueFamilyIndex_Compute(),
                                            VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT);
            if (this->QueueFamilyIndex_Presentation() != VK_QUEUE_FAMILY_IGNORED &&
                ...
                commandPool_presentation->Create(this->QueueFamilyIndex_Presentation(),
                                                 VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT);
                commandPool_presentation->AllocateBuffers(*commandBuffer_presentation);
            }
            for (size_t i = 0; i < std::size(formatProperties); i++)
                vkGetPhysicalDeviceFormatProperties(this->PhysicalDevice(), VkFormat(i), &formatProperties[i]);
        }

        void Cleanup() {
            delete commandPool_graphics;
            delete commandPool_presentation;
            delete commandPool_compute;
            delete commandBuffer_transfer;
            delete commandBuffer_presentation;

            commandPool_graphics = nullptr;
            commandPool_presentation = nullptr;
            commandPool_compute = nullptr;
            commandBuffer_transfer = nullptr;
            commandBuffer_presentation = nullptr;
        }
        ...
        result_t ExecuteCommandBuffer_Graphics(VkCommandBuffer commandBuffer) const {
            fence mfence(const_cast<VkDevice *>(&device));
            VkSubmitInfo submitInfo = {
                    .commandBufferCount = 1,
                    .pCommandBuffers = &commandBuffer
            };
            VkResult result = SubmitCommandBuffer_Graphics(submitInfo, mfence);
            if (!result)
                mfence.Wait();
            return result;
        }
        ...
        result_t ExecuteCommandBuffer_Compute(VkCommandBuffer commandBuffer) const {
            ...
            VkResult result = SubmitCommandBuffer_Compute(submitInfo, mfence);
            if (!result)
                mfence.Wait();
            return result;
        }
        ...
        result_t AcquireImageOwnership_Presentation(VkSemaphore semaphore_renderingIsOver,
            ...
            if (VkResult result = commandBuffer_presentation->Begin(VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT))
                return result;
            CmdTransferImageOwnership(*commandBuffer_presentation);
            if (VkResult result = commandBuffer_presentation->End())
                return result;
            return SubmitCommandBuffer_Presentation(*commandBuffer_presentation,
    ...
    public:
        ...
        void AddCallback_CreateDevice(std::function<void()> function) {
            callbacks_createDevice.push_back(std::move(function));
        }

        void AddCallback_DestroyDevice(std::function<void()> function) {
            callbacks_destroyDevice.push_back(std::move(function));
        }
        ...
        result_t
        CheckDeviceExtensions(arrayRef<const char *> extensionsToCheck, const char *layerName = nullptr) const {
            uint32_t extensionCount;
            std::vector<VkExtensionProperties> availableExtensions;
            if (VkResult result = vkEnumerateDeviceExtensionProperties(physicalDevice, layerName, &extensionCount,
                                                                       nullptr)) {
                layerName ?
                outStream << std::format(
        ...
        result_t CreateSwapchain(bool limitFrameRate = true, VkSwapchainCreateFlagsKHR flags = 0) {
            ...
            if (VkResult result = vkGetPhysicalDeviceSurfacePresentModesKHR(physicalDevice, surface,
